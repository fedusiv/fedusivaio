...
void send_lines(int ypos, uint16_t *linedata)
{
    ...
    for (x=0; x<6; x++) {
        ret=spi_device_queue_trans(spi_handle, &trans[x], portMAX_DELAY);
        assert(ret==ESP_OK);
    }
...
{
    int dc=(int)t->user;
    gpio_set_level(DISPLAY_DC_PIN , dc);
}
...
void init_display()
{
    ...
    io_conf.pin_bit_mask = ((1ULL<<DISPLAY_RST_PIN) | (1ULL<<DISPLAY_DC_PIN) | (1ULL << DISPLAY_BCLK_PIN));
